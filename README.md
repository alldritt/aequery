...
| `undefined-type`, `undefined-command-type` | info | A property, or a command parameter/result, names a type that isn't defined in the SDEF *and* isn't found in any system SDEF (Foundation's Intrinsics, the Cocoa Standard suite, or OpenScripting's Compatibility catalog). Types that resolve against those system files are no longer reported. What remains is a raw four-character type code (valid, but tooling can't display it) or a genuine typo / missing definition. |
